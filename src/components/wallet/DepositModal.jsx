...
function CopyRow({ label, value }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-[14px] bg-[var(--color-surface-2)] px-4 py-3">
      ...
      <button type="button" onClick={copy} aria-label={`Copy ${label}`} className="shrink-0 text-[var(--color-foreground-muted-1)] hover:text-[var(--color-green-1)] cursor-pointer">
...
export function DepositModal({ open, onClose }) {
  ...
  const confirm = () => {
    if (amt < MIN) { toast.error(`Minimum deposit is ₹${MIN}`); return; }
    if (reference.trim().length < 6) { toast.error('Enter a valid UTR / Reference number'); return; }
    dispatch(deposit(amt));
    toast.success(`${fmtINR(amt)} added to your wallet`);
    close();
  };
        ...
        >
          ...
          <div className="flex items-center gap-2 p-4 pb-2">
            <Dialog.Close asChild>
              <button type="button" aria-label="Back" className="flex size-9 items-center justify-center rounded-full text-[var(--color-foreground-muted-1)] hover:bg-[var(--color-surface-2)] hover:text-[var(--color-foreground-primary)] cursor-pointer">
                <ArrowLeft size={18} />
              </button>
            </Dialog.Close>
            ...
            <Dialog.Close asChild>
              <button type="button" aria-label="Close" className="flex size-9 items-center justify-center rounded-full text-[var(--color-foreground-muted-1)] hover:bg-[var(--color-surface-2)] hover:text-[var(--color-foreground-primary)] cursor-pointer">
                <X size={18} />
              </button>
            </Dialog.Close>
          </div>
          <p className="-mt-1 pb-2 text-center text-xs text-[var(--color-foreground-muted-2)]">Balance {fmtINR(balance)}</p>

          {/* Body */}
          <div className="no-scrollbar flex-1 overflow-y-auto px-5 pb-5">
            {/* Amount */}
            <label className="mb-2 block text-[12px] font-bold uppercase tracking-wide text-[var(--color-foreground-primary)]">Amount</label>
            ...
            <div className="mt-5 flex gap-2 rounded-full bg-[var(--color-surface-2)] p-1">
              {[{ k: 'upi', label: 'UPI', Icon: Smartphone }, { k: 'bank', label: 'Bank Transfer', Icon: Landmark }].map(({ k, label, Icon }) => (
                <button
                  key={k}
                  type="button"
                  onClick={() => setMethod(k)}
            ...
            {method === 'upi' && (
              <div className="mt-4 flex flex-col items-center gap-3">
                ...
                <p className="text-center text-xs text-[var(--color-foreground-muted-2)]">Scan with any UPI app, or pay to the UPI ID below</p>
                <div className="w-full"><CopyRow label="UPI ID" value={UPI_ID} /></div>
              </div>
            )}
            ...
            {method === 'bank' && (
              <div className="mt-4 flex flex-col gap-2">
                <CopyRow label="Account Holder" value={BANK.name} />
                <CopyRow label="Account Number" value={BANK.account} />
                <CopyRow label="IFSC Code" value={BANK.ifsc} />
                <CopyRow label="Bank" value={BANK.bank} />
              </div>
            )}

            {/* Manual deposit: reference */}
            <label className="mb-2 mt-5 block text-[12px] font-bold uppercase tracking-wide text-[var(--color-foreground-primary)]">UTR / Reference No.</label>
            <input
              value={reference}
            ...
            <label className="mb-2 mt-5 block text-[12px] font-bold uppercase tracking-wide text-[var(--color-foreground-primary)]">
              Payment Screenshot <span className="font-medium normal-case text-[var(--color-foreground-muted-2)]">(optional)</span>
            </label>
            {screenshot ? (
              <div className="flex items-center gap-3 rounded-[14px] bg-[var(--color-surface-2)] p-3">
                <img src={URL.createObjectURL(screenshot)} alt="proof" className="size-12 shrink-0 rounded-[10px] object-cover" />
                <span className="flex-1 truncate text-[13px] font-semibold text-[var(--color-foreground-primary)]">{screenshot.name}</span>
                <button type="button" onClick={() => setScreenshot(null)} aria-label="Remove screenshot" className="shrink-0 text-[var(--color-foreground-muted-1)] hover:text-[var(--color-danger)] cursor-pointer">
                  <X size={18} />
                </button>
              </div>
            ) : (
              <label className="flex cursor-pointer flex-col items-center justify-center gap-1.5 rounded-[14px] border-2 border-dashed border-[var(--color-foreground-muted-1)]/20 py-5 text-[var(--color-foreground-muted-2)] transition-colors hover:border-[var(--color-green-1)]/50 hover:text-[var(--color-foreground-muted-1)]">
                <Upload size={20} />
                <span className="text-[13px] font-semibold">Upload payment screenshot</span>
                <span className="text-[11px]">PNG / JPG</span>
            ...
            <div className="mt-3 rounded-[14px] bg-[var(--color-surface-2)]/60 px-4 py-3 text-[12px] leading-relaxed text-[var(--color-foreground-muted-2)]">
              Pay the amount via {method === 'upi' ? 'UPI' : 'bank transfer'}, then enter the UTR / reference number and confirm. Your balance updates instantly.
            </div>
            ...
            >
              Confirm Deposit{amt >= MIN ? ` · ${fmtINR(amt)}` : ''}
            </button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
