...
export function RegisterForm({ onSwitchToSignin }) {
  ...
  const validateStep1 = () => {
    const next = {};
    if (!formData.email.trim()) next.email = t('auth.emailRequired', 'Email is required');
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) next.email = t('auth.enterValidEmail', 'Please enter a valid email');
    setErrors(next);
  ...
  const validateStep2Username = () => {
    ...
    if (formData.username.trim()) {
      if (formData.username.length < 5) next.username = t('auth.usernameMinLength', 'Username must be at least 5 characters');
      else if (formData.username.length > 16) next.username = t('auth.usernameMaxLength', 'Username must be less than 16 characters');
      else if (formData.email && formData.email.split('@')[0].toLowerCase() === formData.username.toLowerCase()) {
        next.username = t('auth.usernameEmailSame', 'Username and email name cannot be the same');
      }
    }
    setErrors(next);
    return Object.keys(next).length === 0;
  ...
  const validateStep3 = () => {
    ...
    else if (formData.password.length < 8) next.password = t('auth.passwordMinLength8', 'Password must be at least 8 characters');
    else if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/.test(formData.password)) {
      next.password = t('auth.passwordComplexity', 'Password must contain uppercase, lowercase, number, and special character');
    }
    if (!formData.confirmPassword) next.confirmPassword = t('auth.confirmYourPassword', 'Please confirm your password');
    else if (formData.password !== formData.confirmPassword) next.confirmPassword = t('auth.passwordsDoNotMatch', 'Passwords do not match');
    setErrors(next);
    return Object.keys(next).length === 0;
  };
  ...
  const validateStep4 = () => {
    const next = {};
    if (!formData.ruleChecked) next.ruleChecked = t('auth.mustAgreeTerms', 'You must agree to the Terms of Service');
    setErrors(next);
    return Object.keys(next).length === 0;
  };
  ...
  const onStep1Submit = (e) => {
    e.preventDefault();
    if (!validateStep1()) {
      showError(t('auth.enterValidEmailAddress', 'Please enter a valid email address.'));
      return;
    }
    setStep(2);
  };
  ...
  const onStep2Submit = (e) => {
    ...
    if (!validateStep2Username()) {
      showError(t('auth.pleaseFixUsername', 'Please fix the username.'));
      return;
    }
    setStep(3);
  };
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!validateStep4()) {
      showError(t('auth.mustAgreeTermsPrivacy', 'You must agree to the Terms of Service and Privacy Policy.'));
      return;
    }
  ...
  if (step === 1) {
    return (
      <form onSubmit={onStep1Submit} className="flex flex-col gap-5">
        <p className="text-[var(--color-foreground-primary)]">{t('auth.enterYourEmailAddress', 'Enter your email address:')}</p>
  ...
  if (step === 2) {
    return (
      <form onSubmit={onStep2Submit} className="flex flex-col gap-5">
        ...
        <p className="text-[var(--color-foreground-primary)]">{t('auth.chooseUsername', 'Choose a username:')}</p>
        <div className="flex flex-col gap-2">
          <input
            type="text"
            name="username"
            ...
            onChange={handleChange}
            placeholder={t('auth.enterUsernameChars', 'Enter username (5–16 characters)')}
            className={inputClass}
            autoComplete="username"
            aria-invalid={!!errors.username}
          />
          {errors.username && <p className="text-sm text-[var(--color-danger)]">{errors.username}</p>}
          <p className="text-xs text-[var(--color-foreground-muted-1)]">{t('auth.usernameOptionalHint', 'Optional. Leave blank to use your email.')}</p>
        </div>
        <Button type="submit" variant="primary" className="w-full py-3 uppercase">
          {t('auth.continue', 'Continue')}
        </Button>
  ...
  if (step === 3) {
    return (
      <form onSubmit={onStep3Submit} className="flex flex-col gap-5">
        ...
        >
          {t('auth.changeUsername', '← Change username')}
        </button>
        <p className="text-[var(--color-foreground-primary)]">{t('auth.createPassword', 'Create a password:')}</p>
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-[var(--color-foreground-primary)]">{t('auth.password', 'Password')}</label>
          <PasswordInput
            name="password"
            value={formData.password}
            onChange={handleChange}
            placeholder={t('auth.enterPasswordMin8', 'Enter password (min 8 characters)')}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-[var(--color-foreground-primary)]">{t('auth.confirmPassword', 'Confirm Password')}</label>
          <PasswordInput
            name="confirmPassword"
            value={formData.confirmPassword}
            onChange={handleChange}
            placeholder={t('auth.confirmYourPasswordPlaceholder', 'Confirm your password')}
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <h2 className="text-xl font-semibold uppercase text-[var(--color-foreground-primary)]">{t('auth.signUpHeading', 'Sign Up')}</h2>
      <p className="text-sm text-[var(--color-foreground-muted-1)]">{t('auth.almostDone', 'Almost done. Add a referral code (optional) and accept the terms.')}</p>

      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-[var(--color-foreground-primary)]">{t('auth.referralCodeOptional', 'Referral code (optional)')}</label>
        <input
          type="text"
          name="referral"
      ...
      <p className="text-xs text-[var(--color-foreground-muted-1)]">{t('auth.emailLabel', 'Email: {{email}}', { email: formData.email })}</p>

      {showOtpSection && (
        <div className="flex flex-col gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
          <p className="text-sm text-[var(--color-foreground-primary)]">{t('auth.otpSentToEmail', 'We sent a verification code to your email.')}</p>
          <button
            type="button"
          ...
          >
            {resendTimer > 0 ? t('auth.resendIn', 'Resend in {{seconds}}s', { seconds: resendTimer }) : t('auth.resendOtp', 'Resend OTP')}
          </button>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-[var(--color-foreground-primary)]">{t('auth.verificationCodeLabel', 'Verification code')}</label>
            <input
              type="text"
              value={otp}
      ...
      <div className="flex flex-col gap-1">
        <label className="flex cursor-pointer items-start gap-2">
          ...
          <span className="text-sm text-[var(--color-foreground-muted-1)]">
            ...
            <a href="/terms" target="_blank" rel="noopener noreferrer" className="underline hover:opacity-90">{t('auth.termsOfService', 'Terms of Service')}</a>
            {' '}{t('auth.and', 'and')}{' '}
            <a href="/privacy" target="_blank" rel="noopener noreferrer" className="underline hover:opacity-90">{t('auth.privacyPolicy', 'Privacy Policy')}</a>
          </span>
        </label>
      ...
      <div className="flex items-start gap-2">
        <input
          ...
          disabled={isLoading}
        />
        <span className="text-sm text-[var(--color-foreground-muted-1)]">{t('auth.agreeMarketing', 'I agree to receive marketing communications')}</span>
      </div>

      <Button type="submit" variant="primary" className="w-full py-3 uppercase" disabled={isLoading}>
        {isLoading ? t('auth.creatingAccount', 'Creating account...') : t('auth.signUpButton', 'Sign Up')}
      </Button>
      <SocialLoginButtons
